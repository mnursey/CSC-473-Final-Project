...
public class CrawlerAgent : Agent
{
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        for(int i = 0; i < numberOfVisionRaycasts; i++ )
        {
            ...
            if (Physics.Raycast(body.position, Quaternion.Euler(0, angle, 0) * body.forward * maxRaycastDist, out hit, maxRaycastDist, raycastMask))
            {
                sensor.AddObservation(hit.distance / maxRaycastDist);
            ...
            {
                Debug.DrawRay(body.position, Quaternion.Euler(0, angle, 0) * body.forward * maxRaycastDist, Color.green);
                sensor.AddObservation(1);
            }
        }
